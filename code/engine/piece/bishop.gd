...
func calc_moves(pos, piece, game):
	...
	# Get movements.
	linear.add_moves(positions, piece.team, pos + game.get("width") + 1, pos + (game.get("width") - start_file - 1) * (game.get("width") + 1),
			game.get("width") + 1, game.get("max_pos"), game.pieces)
	linear.add_moves(positions, piece.team, pos + game.get("width") - 1, pos + (start_file) * (game.get("width") - 1),
			game.get("width") - 1, game.get("max_pos"), game.pieces)
	linear.add_moves(positions, piece.team, pos - game.get("width") - 1, pos + (start_file) * (-1 - game.get("width")),
			-1 - game.get("width"), game.get("max_pos"), game.pieces)
	linear.add_moves(positions, piece.team, pos - game.get("width") + 1, pos + (game.get("width") - start_file - 1) * (1 - game.get("width")),
			1 - game.get("width"), game.get("max_pos"), game.pieces)
